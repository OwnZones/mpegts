...
void PATHeader::encode(SimpleBuffer &rSb) {
    ...
    rSb.write_2bytes(mTransportStreamId);
    ...
    rSb.write_1byte(mSectionNumber);
    rSb.write_1byte(mLastSectionNumber);
}

void PATHeader::decode(SimpleBuffer &rSb) {
    mTableId = rSb.read_1byte();
    ...
    mTransportStreamId = rSb.read_2bytes();

    uint8_t lB5 = rSb.read_1byte();
    mReserved1 = (lB5 >> 6) & 0x03;
    mVersionNumber = (lB5 >> 1) & 0x1F;
    mCurrentNextIndicator = lB5 & 0x01;

    mSectionNumber = rSb.read_1byte();

    mLastSectionNumber = rSb.read_1byte();
}
...
void PMTHeader::encode(SimpleBuffer &rSb) {
    ...
    rSb.write_2bytes(mProgramNumber);
...
void PMTHeader::decode(SimpleBuffer &rSb) {
    ...
    mProgramNumber = rSb.read_2bytes();
...
void AdaptationFieldHeader::encode(SimpleBuffer &rSb) {
    rSb.write_1byte(mAdaptationFieldLength);
...
void AdaptationFieldHeader::decode(SimpleBuffer &rSb) {
    mAdaptationFieldLength = rSb.read_1byte();
    if (mAdaptationFieldLength != 0) {
        uint8_t lVal = rSb.read_1byte();
...
void PESHeader::encode(SimpleBuffer &rSb) {
    ...
    rSb.write_4bytes(lB0b1b2b3);

    rSb.write_2bytes(mPesPacketLength);
    ...
    rSb.write_1byte(mHeaderDataLength);
}

void PESHeader::decode(SimpleBuffer &rSb) {
    uint32_t lB0b1b2b3 = rSb.read_4bytes();
    mPacketStartCode = (lB0b1b2b3 >> 8) & 0x00FFFFFF;
    mStreamId = (lB0b1b2b3) & 0xFF;

    mPesPacketLength = rSb.read_2bytes();

    uint8_t lB6 = rSb.read_1byte();
    ...
    mHeaderDataLength = rSb.read_1byte();
}
